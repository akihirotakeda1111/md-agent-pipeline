...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Additional pytest argument; repeatable and intended for diagnostics",
    )
...
def preflight(production_root: Path, binding: str) -> list[str]:
    ...
    if importlib.util.find_spec("pytest") is None:
        errors.append("pytest is not installed; run: python -m pip install -r requirements.txt")
    if importlib.util.find_spec("yaml") is None:
        errors.append("PyYAML is not installed; run: python -m pip install -r requirements.txt")
    if shutil.which("git") is None:
        errors.append("git executable was not found on PATH")
    workflow = production_root / ".github" / "workflows" / "agent-execute.yml"
...
def write_report(report_dir: Path, payload: dict[str, Any]) -> None:
    ...
    (report_dir / "phase6-result.json").write_text(
...
def main() -> int:
    ...
    if args.list:
        for case in cases:
            print(f"{case['id']:>3}  {case['area']:<18} {case['title']}")
        return 0
    ...
    env = dict(os.environ)
    env["PHASE6_BINDING"] = args.binding
    env["PHASE6_PRODUCTION_ROOT"] = str(production_root)
    env["PYTHONPATH"] = os.pathsep.join(
        [str(ROOT), str(production_root), env.get("PYTHONPATH", "")]
    ).rstrip(os.pathsep)
    print(f"[Phase 6] Production root: {production_root}")
